...
public class GetInstallmentsCustomerIntegrationTest {
    ...
    @Test
    public void shouldGetInstallmentsIfUserExistsLoanExistsPendingPayments() {
        ...
        HttpHeaders getInstallmentsRequestHeaders = new HttpHeaders();
        getInstallmentsRequestHeaders.add(Constants.USERNAME_HEADER, "abc");
        getInstallmentsRequestHeaders.add(Constants.PASSWORD_HEADER, "123");
        HttpEntity<PayInstallmentRequest> payInstallmentRequestHttpEntity =
                new HttpEntity<>(null, getInstallmentsRequestHeaders);
        ResponseEntity<List<Installment>> response = restTemplate.exchange(
                createURLWithPort("/api/aspire/installment", 8080),
                HttpMethod.GET, payInstallmentRequestHttpEntity, new ParameterizedTypeReference<List<Installment>>() {});
